...
var min_denomations = [
{num:8,price:0,packid:"27545030469"},
{num:8,price:0,packid:"27545053701"},
...
function calc_and_display_price(){
    ...
    document.getElementById('rs-maxme-cost').innerHTML = "$" + (total/100).toFixed(2) + " USD"
    document.getElementById('rs-buy-button').innerHTML = "BUY " + countTrues + "/8";
    document.getElementById('rs-buy-button').onclick = function(){
        var block = "https://realmstock.com/cart/";
        var elements = [];
...
function recursive_helper(memtotal,memids,list_of_stats){
	if(count_min(0,list_of_stats)==8){
		return [memtotal,memids];
	}

	for(var g = 0; g < list_of_stats.length; g++){
		if(list_of_stats[g] > 0){

			var max_via_1pack = list_of_stats.slice();
			max_via_1pack[g] = 0;
			var ids_add_1pack = memids.slice();
			ids_add_1pack.push(min_denomations[g].packid);
			var maxed_via_singlepack = recursive_helper(memtotal + min_denomations[g].price,ids_add_1pack,max_via_1pack)

			var max_via_rainbowpack = list_of_stats.slice();
			var amntOfPotsNeeded = Math.ceil(list_of_stats[g]/(g==0||g==1?5:1))
			for(var m in max_via_rainbowpack)max_via_rainbowpack[m] -= amntOfPotsNeeded*(m==0||m==1?5:1);
			var ids_add_rainbowpack = memids.slice();
			for(var h = 0; h < amntOfPotsNeeded;h++)ids_add_rainbowpack.push("37667831237");
			var maxed_via_rainbowpack = recursive_helper(memtotal + 159*amntOfPotsNeeded,ids_add_rainbowpack,max_via_rainbowpack)

			if(maxed_via_singlepack[0] > maxed_via_rainbowpack[0])return maxed_via_rainbowpack;
			else return maxed_via_singlepack;

		}
	}
}

function add_to_mem(mem,packID){
	if(packID in mem)mem[packID]++;
	else mem[packID]=1;
}
